...
- `/v2/sd/team/.../access-token` 请求的 `https://主机名` 填入 `CLICKUP_ACCESS_TOKEN_BASE`，不要包含后面的路径。
- `/graphql/gateway` 请求的 `https://主机名` 填入 `CLICKUP_BASE_URL`，不要包含 `/graphql/gateway`；WebSocket 地址会由它自动从 `https://` 转换为 `wss://`。
